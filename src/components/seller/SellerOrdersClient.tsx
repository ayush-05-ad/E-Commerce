...
import { ShoppingBag, ChevronDown, ChevronUp, MapPin, Truck, AlertCircle } from "lucide-react";
...
export function SellerOrdersClient({ initialOrders }: SellerOrdersClientProps) {
    ...
    <div className="space-y-8">
      ...
      {orders.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-neutral-300 dark:border-neutral-800 bg-white dark:bg-neutral-950 p-12 text-center flex flex-col items-center justify-center gap-4">
          ...
          <h3 className="font-extrabold text-base text-neutral-900 dark:text-white">No orders placed</h3>
          <p className="text-xs text-neutral-500 max-w-sm">
            We haven&apos;t received any transactions for your products yet. Check back here once customers start buying!
          </p>
        </div>
      ) : (
                ...
                {orders.map((order) => {
                  ...
                  return (
                    <>
                      ...
                      >
                        ...
                        <td className="py-4 px-6" onClick={(e) => e.stopPropagation()}>
                          <select
                            ...
                            className="bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg px-2.5 py-1.5 text-xxs font-bold uppercase tracking-wider focus:outline-none focus:ring-1 focus:ring-neutral-900 text-neutral-900 dark:text-white"
                          >
                      ...
                      {/* Expandable Shipment Details Details Drawer */}
                      {isExpanded && (
                        <tr className="bg-neutral-50/50 dark:bg-neutral-900/30 border-b border-neutral-100 dark:border-neutral-900 animate-in fade-in duration-300">
                          <td colSpan={6} className="py-6 px-12 space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                              ...
                              <div className="space-y-3">
                                <h4 className="font-black text-xs text-neutral-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                                  <MapPin className="w-4 h-4 text-neutral-400" /> Shipping Details
                                </h4>
                                {order.address ? (
